...
//    LCD_DisplayStringLine(Line1, (uint8_t *)"                    ");
    LCD_DisplayStringLine(Line0, (uint8_t *)" WWW.NJTECH.EDU.CN  ");
    LCD_DisplayStringLine(Line1, (uint8_t *)" HAL LIB@80Mhz 1017 ");
...
void  PulseMeasure_Test(void)
{
    ...
    if((uwTick-PWM_uwTick)>1000)
    {
        ...
        sprintf((char *)Lcd_Disp_String," TIM2:%.2fKHz,%.1f%%",Tim2_Fre,Tim2_Duty);
        LCD_DisplayStringLine(Line6, (uint8_t *)Lcd_Disp_String);
        //显示TIM3即PB4管脚输入的脉冲频率和占空比
        sprintf((char *)Lcd_Disp_String," TIM3:%.2fKHz,%.1f%%",Tim3_Fre,Tim3_Duty);
...
{
    sprintf((char *)Lcd_Disp_String," DA1:%.1fV;DA2:Triangle ",DAC1_CH1_Vol);
